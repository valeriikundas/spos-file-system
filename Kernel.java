...
/**
 * Simulates a unix-like file system. Provides basic directory and file
 * operations and implements them in terms of the underlying disk block
 * structures.
 * 
 * @author Ray Ontko
 */
public class Kernel {
  ...
  /**
   * The name this program uses when displaying any error messages it generates
   * internally.
   */
   ...
   * </pre>
   */
  public static final String[] sys_errlist = { null, "Not owner", "No such file or directory", null, null, null, null,
      null, null, "Bad file number", null, null, null, "Permission denied", null, null, null, "File exists",
      "Cross-device link", null, "Not a directory", "Is a directory", "Invalid argument", "File table overflow",
      "Too many open files", null, null, "File too large", "No space left on device", null, "Read-only file system",
      "Too many links" };

  /**
   * Prints a system error message. The actual text written to stderr is the given
   * string, followed by a colon, a space, the message text, and a newline. It is
   * customary to give the name of the program as the argument to perror.
   * 
   ...
   * @return Zero if the file is closed; -1 if the file descriptor is invalid.
   */
   ...
   * <p>
   * Creates a new file or prepares to rewrite an existing file. If the file does
   * not exist, it is given the mode specified. If the file does exist, it is
   * truncated to length zero. The file is opened for writing and its file
   ...
   * @return the file descriptor (a non-negative integer); -1 if a needed
   *         directory is not searchable, if the file does not exist and the
   *         directory in which it is to be created is not writable, if the file
   *         does exist and is unwritable, if the file is a directory, or if there
   *         are already too many open files.
   * @exception java.lang.Exception if any underlying action causes an exception
   *                                to be thrown
   */
  public static int creat(String pathname, short mode) throws Exception {
    // get the full path
    ...
    if (indexNodeNumber < 0) {
      ...
      System.out.println("here setting uid=" + process.getUid() + " and gid=" + process.getGid());
   ...
   * @exception java.lang.Exception if any underlying Exception is thrown
   */
  ...
  /**
   * Set the current file pointer for a file. The current file position is updated
   * based on the values of offset and whence. If whence is 0, the new position is
   * offset bytes from the beginning of the file. If whence is 1, the new position
   * is the current position plus the value of offset. If whence is 2, the new
   * position is the size of the file plus the offset value. Note that offset may
   * be negative if whence is 1 or 2, as long as the resulting position is not
   * less than zero. It is valid to position past the end of the file, but it is
   * not valid to read past the end of the file.
   * <p>
   * Simulates the unix system call:
   * 
   * <pre>
   ...
   * @param whence 0 = from beginning of file; 1 = from current position ; 2 =
   *               from end of file
   */
   ...
   * descriptor must be used for subsequent calls for other input and output
   ...
   * 
   * @param pathname the name of the file or directory to create
   * @param flags    the flags to use when opening the file: O_RDONLY, O_WRONLY,
   *                 or O_RDWR.
   * @return the file descriptor (a non-negative integer); -1 if the file does not
   *         exist, if one of the necessary directories does not exist or is
   *         unreadable, if the file is not readable (resp. writable), or if too
   *         many files are open.
   * @exception java.lang.Exception if any underlying action causes an exception
   *                                to be thrown
   */
  public static int open(String pathname, int flags) throws Exception {
    ...
    // set up the file descriptor
    FileDescriptor fileDescriptor = new FileDescriptor(openFileSystems[ROOT_FILE_SYSTEM], indexNode, flags);
    fileDescriptor.setIndexNodeNumber(indexNodeNumber);
  ...
  /**
   * Open a file using a FileDescriptor. The open and create methods build a file
   * descriptor and then invoke this method to complete the open process.
   * <p>
   * This is a convenience method for the simulator kernel.
   * 
   * @param fileDescriptor the file descriptor
   * @return the file descriptor index in the process open file list assigned to
   *         this open file
   */
   ...
   * @exception java.lang.Exception if any underlying action causes Exception to
   *                                be thrown
   */
  public static int read(int fd, byte[] buf, int count) throws Exception {
    // check fd
   ...
   *                                be thrown
   */
  public static int fstat(int fd, Stat buf) throws Exception {
    // check fd
    int status = check_fd(fd);
    if (status < 0)
      return status;
   ...
   *                                be thrown
   */
  public static int stat(String name, Stat buf) throws Exception {
   ...
   *                                be thrown
   */
  public static int write(int fd, byte[] buf, int count) throws Exception {
    // check fd
  ...
  /*
   * to be done: int access(const char *pathname, int mode); int link(const char
   * *oldpath, const char *newpath); int unlink(const char *pathname); int
   * rename(const char *oldpath, const char *newpath); int symlink(const char
   * *oldpath, const char *newpath); int lstat(const char *file_name, struct stat
   * *buf); int chmod(const char *path, mode_t mode); int fchmod(int fildes,
   * mode_t mode); int chown(const char *path, uid_t owner, gid_t group); int
   * fchown(int fd, uid_t owner, gid_t group); int lchown(const char *path, uid_t
   * owner, gid_t group); int utime(const char *filename, struct utimbuf *buf);
   ...
   * current ProcessContext. If multiple processes are implemented, then this
   * variable will "point" to different processes at different times.
   */
  ...
  // ??? should be private?
  public static FileSystem[] openFileSystems = new FileSystem[MAX_OPEN_FILE_SYSTEMS];

  // ??? should be private?
  public static short ROOT_FILE_SYSTEM = 0;

  /**
   * Initialize the file simulator kernel. This should be the first call in any
   * simulation program. You can think of this as the method which "boots" the
   * kernel. This method opens the "filesys.conf" file (or the file named by the
   * system property "filesys.conf") and reads any properties given in that file,
   * including the filesystem.root.filename and filesystem.root.mode ("r", "rw").
   */
  public static void initialize() {
    ...
    String rootFileSystemFilename = properties.getProperty("filesystem.root.filename", "filesys.dat");
  ...
  /**
   * End the simulation and exit. Terminates any remaining "processes", flushes
   * all file system blocks to "disk", and exit the simulation program. This
   * method is generally called by exit() when the last process terminates.
   * However, it may also be called directly to gracefully end the simlation.
   * 
   * @param status the status to use with System.exit()
   * @exception java.lang.Exception if any underlying operation causes and
  ...
  /**
   * Check to see if the integer given is a valid file descriptor index for the
   * current process. Sets errno to EBADF if invalid.
   * <p>
   * This is a convenience method for the simulator kernel; it should not be
   * called by user programs.
   * 
   * @param fd the file descriptor index
   * @return zero if the file descriptor index is valid; -1 if the file descriptor
   *         index is not valid
   */
  private static int check_fd(int fd) {
    // look for the file descriptor in the open file list
    if (fd < 0 || fd >= process.openFiles.length || process.openFiles[fd] == null) {
  ...
  /**
   * Check to see if the integer given is a valid file descriptor index for the
   * current process, and if so, whether the file is open for reading. Sets errno
   * to EBADF if invalid or not open for reading.
   * <p>
   * This is a convenience method for the simulator kernel; it should not be
   * called by user programs.
   * 
   * @param fd the file descriptor index
   * @return zero if the file descriptor index is valid; -1 if the file descriptor
   *         index is not valid or is not open for reading.
   */
  ...
  /**
   * Check to see if the integer given is a valid file descriptor index for the
   * current process, and if so, whether the file is open for writing. Sets errno
   * to EBADF if invalid or not open for writing.
   * <p>
   * This is a convenience method for the simulator kernel; it should not be
   * called by user programs.
   * 
   * @param fd the file descriptor index
   * @return zero if the file descriptor index is valid; -1 if the file descriptor
   *         index is not valid or is not open for writing.
   */
  ...
  /**
   * Get the full path for a file by adding the working directory for the current
   * process to the beginning of the given path name if necessary.
   * 
  ...
  private static short findNextIndexNode(FileSystem fileSystem, IndexNode indexNode, String name,
  ...
  private static short findIndexNode(String path, IndexNode inode) throws Exception {
    ...
    while (st.hasMoreTokens()) {
      ...
      if (!s.equals("")) {
        ...
        indexNodeNumber = findNextIndexNode(fileSystem, indexNode, s, nextIndexNode);
        if (indexNodeNumber < 0) {
          // return ENOENT
          process.errno = ENOENT;
          return -1;
  ...
  public static int chown(String path, short uid, short gid) throws Exception {
    ...
    if (gid >= 0) {
      if (indexNode.getUid() == process.getUid() || process.getUid() == 0) {
        ...
        Kernel.perror(PROGRAM_NAME);
        System.err.println(PROGRAM_NAME + ": you do not have access");
        Kernel.exit(2);
      }
    }
