...
public class FileSystem {
   ...
   * @exception java.io.IOException if any IOExceptions are thrown during the
   *                                open.
   */
  public FileSystem(String newFilename, String newMode) throws IOException {
    super();
   ...
   * 
   * @exception java.io.IOException if the open or read causes IOException to be
   *                                thrown
   */
   ...
   * 
   * @exception java.io.IOException if the closing the backing file causes any
  ...
  /**
   * Read bytes into a buffer from the specified absolute block number of the file
   * system.
   * 
   ...
   * @exception java.io.IOException if there are any exceptions during the read
   *                                from the underlying "file system" file.
   */
  ...
  /**
   * Write bytes from a buffer to the specified absolute block number of the file
   * system.
   * 
   ...
   * @exception java.io.IOException if there are any exceptions during the write
   *                                to the underlying "file system" file.
   */
   ...
   * @exception java.io.IOException if any exception occurs during an operation on
   *                                the underlying "file system" file.
   */
   ...
   * 
   * @return the data block number which was allocated; -1 if no blocks are
   *         available
   * @exception java.io.IOException if any exception occurs during an operation on
   *                                the underlying "file system" file.
   */
  public int allocateBlock() throws IOException {
    ...
    while (true) {
      ...
      boolean allocated = freeListBitBlock.isBitSet(currentFreeListBitNumber % (blockSize * 8));
      ...
      if (!allocated) {
        freeListBitBlock.setBit(previousFreeListBitNumber % (blockSize * 8));
        file.seek((freeListBlockOffset + currentFreeListBlock) * blockSize);
        freeListBitBlock.write(file);
  ...
  /**
   * Loads the block containing the specified data block bit into the free list
   * block buffer. This is a convenience method.
   * 
   * @param dataBlockNumber the data block number
   * @exception java.io.IOException
   */
  private void loadFreeListBlock(int dataBlockNumber) throws IOException {
    int neededFreeListBlock = dataBlockNumber / (blockSize * 8);

    if (currentFreeListBlock != neededFreeListBlock) {
      file.seek((freeListBlockOffset + neededFreeListBlock) * blockSize);
  ...
  /**
   * The index node number that will next be checked to see if it is available.
   */
  private short currentIndexNodeNumber = 0;

  /**
   * The number of the index node block which is currently loaded into
   * indexBlockBytes. If no block is loaded, this contains the value "-1".
   */
  private short currentIndexNodeBlock = -1;

  /**
   * The byte buffer used for reading and writing index node blocks. You can think
   * of this as a one-block cache.
   */
   ...
   * 
   * @return the inode number for the next available index node; -1 if there are
   *         no index nodes available.
   * @exception java.io.IOException if there is an exception during an operation
   *                                on the underlying "file system" file.
   */
   ...
   * @execption java.io.IOException if any exception occurs in an underlying
   *            operation on the "file system" file.
   */
  public void readIndexNode(IndexNode indexNode, short indexNodeNumber) throws IOException {
   ...
   *            operation on the "file system" file.
   */
  public void writeIndexNode(IndexNode indexNode, short indexNodeNumber) throws IOException {
    loadIndexNodeBlock(indexNodeNumber);
  ...
  /**
   * Loads the block containing the specified index node into the index node block
   * buffer. This is a convenience method.
   * 
   * @param indexNodeNumber the index node number
   * @exception java.io.IOException
   */
  private void loadIndexNodeBlock(short indexNodeNumber) throws IOException {
